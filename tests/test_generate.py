...
def test_fixed_amount():
    ...
    fixed_number = get_random_selection_with_surge('C3H6', 1)
    assert len(fixed_number) == 1

    # Test only a certain fraction
    fixed_frac = get_random_selection_with_surge('C3H6', 0.5)
    assert len(fixed_frac) == 1

    # Test a bigger case
    larger_case = get_random_selection_with_surge('C6H12', to_select=10)
